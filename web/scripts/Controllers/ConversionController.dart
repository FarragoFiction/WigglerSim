...
Future<Null> begin() async{
  ...
  printToScreen("First, we need to convert your caretaker, ${game.player.name}.");
    game.player.doll.name = game.player.name;
    await new Future.delayed(const Duration(milliseconds : 500));
    printToScreen("Done! Now it's time for your wigglers.... Don't worry, I'll treat them like my own subroutines.");
    for(Pet p in game.player.petInventory.pets) {
      p.doll.name = p.name;
      await new Future.delayed(const Duration(milliseconds : 100));
      printToScreen("${p.name} is done!");
    }
